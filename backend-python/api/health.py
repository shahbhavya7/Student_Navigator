...
from agents.graph import clr_agent, performance_agent, engagement_agent, curriculum_agent, motivation_agent
...
async def agent_health():
    ...
    # Workflow statistics
    total_executions = sum(a["execution_count"] for a in agents.values())
    total_errors = sum(a["error_count"] for a in agents.values())
    
    workflow_stats = {
        "total_executions": total_executions,
        "total_errors": total_errors,
        "average_success_rate": (
            sum(a["success_rate"] for a in agents.values()) / len(agents)
            if agents else 0
        )
    }
